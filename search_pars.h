...
public:
	...
	void  Srchnd(boost::mutex& m1, boost::condition_variable *hEvent,
		...
		long in, int numprocess, long thread_in[], misc& utils,int sstrat, int noct)
	{
		long inde;
...
public:
	...
	int get_search_pars(registry *reg)
	{
		...
		k = get_key(reg,  ("SEARCH"),  ("NDMIN"));
		if (k)
			ndmin = get_int(k);
		else return -1;

		k = get_key(reg,  ("SEARCH"),  ("NDMAX"));
		if (k)
			ndmax = get_int(k);
		else return -1;
		printf (" Number of hard samples (min/max): %d %d \n",ndmin,ndmax);

		k = get_key(reg,  ("SEARCH"),  ("NODMAX"));
		if (k)
			nodmax = get_int(k);
		else return -1;
		logger << " Max. previous simulated nodes: "<< utilities::make_string(nodmax) <<"\n";

		k = get_key(reg,  ("SEARCH"),  ("SSTRAT"));
		if (k)
			sstrat = get_int(k);
		else return -1;
		if (sstrat == 1) ndmax = 0;
