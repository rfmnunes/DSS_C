...
public:
	...
	int read_miscelaneous_parameters(registry *reg)
	{
		...
		k = get_key(reg,  ("SEARCH"),  ("MULTS"));
		if (k)
			mults = get_int(k);
		else return -1;

		k = get_key(reg,  ("SEARCH"),  ("NMULTS"));
		if (k)
			nmult = get_int(k);
		else return -1;
		...
		if ((k = get_key(reg,  "SIMULATION",  "OUTFILE")) != NULL)
			outfl= get_string(k);
		...
		k = get_key(reg,  ("SIMULATION"),  ("NSIMS"));
		if (k)
			nsim = get_int(k);
		else return -1;
		...
		k = get_key(reg,  ("SIMULATION"),  ("NTRY"));
		if (k)
			ntry = get_int(k);
		else return -1;

		k = get_key(reg,  ("SIMULATION"),  ("AVGCORR"));
		if (k)
			icmean = get_int(k);
		else return -1;

		k = get_key(reg,  ("SIMULATION"),  ("VARCORR"));
		if (k)
			icvar = get_int(k);
		else return -1;
		...
		k = get_key(reg,  ("GENERAL"),  ("USEHEADERS"));
		if (k)
			headerflag = get_int(k);
		else return -1;

		// parsing general section
		k = get_key(reg,  ("GENERAL"),  ("NULLVAL"));
		if (k)
			nosvalue= get_float(k);
		else return -1;		

		filetype = SGEMS;;
		if ((k = get_key(reg, "GENERAL", "FILETYPE")) != NULL) 
		{
	...
	void sortit (long ib, long ie, OtherType *a, int iperm,  SomeType *b, stillothertype *c, float d[],
