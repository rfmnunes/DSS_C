...
public:
	...
	std::vector<double>* orig_distr;	//original 
	std::vector<double>* gauss_distr;	// gaussian transformed

	unsigned int n_data;	//number of points
	...
	Distribution(std::vector<double> &distr, std::vector<double> &gauss_distr_in, unsigned int n_data_in, double z_min, double z_max); // check if scope is correct;

	Distribution(const Distribution &in_distr);

	Distribution& operator=(const Distribution& in_distr);
	...
	template <class SomeType>
	static int locate_in_vector_f(std::vector<SomeType> &in_array, unsigned int size, double value);

	double get_limits(double &cmean);
	double get_limits_c(double &cmean);
	double backtr(double &simgval, int &ltail, double &ltpar, int &utail, double &utpar);
	double backtr_trunc(double &simgval);
	double backtr_c(double &simgval, int &ltail, double &ltpar, int &utail, double &utpar);
	double gcum(double &x);
	double powint(double xlow, double xhigh, double &ylow, double &yhigh, double &xval, double potencia);
	void update_gaussian_distribution();
	static double gauinv(double &p);
	static	double RationalApproximation(double t);
	static double NormalCDFInverse(double &p);
	void weighted_add_to_distribution(misc utils, Distribution to_add, float weight);
	double get_distribution_average();
	double get_distribution_variance();
	double get_distribution_median();
};
...
//	Distribution()
//	{
//		orig_distr = new std::vector<double> ;	
//		gauss_distr = new std::vector<double> ;
//
//	};
//
//	Distribution(std::vector<double> &distr,std::vector<double> &gauss_distr_in,unsigned int n_data_in, double z_min, double z_max) // check if scope is correct
