...
//	unsigned int n_distr_pseudo;
	...
	{	
		...
		// pseudohard data
		k = get_key(reg, (char*)"PSEUDOHARD", (char*)"USEPSEUDO");
		if (k)
			usepseudo = get_int(k);
		else return -1;
		...
		if (usepseudo==1)
		{	
			if ((k = get_key(reg, (char*)"PSEUDOHARD", (char*)"PSEUDOFILE")) != NULL)
			pseudofl = get_string(k);
			boost::algorithm::trim(pseudofl);
			std::cout<<" Pseudo hard data file: "<< pseudofl <<"\n";
		}

		k = get_key(reg, (char*)"PSEUDOHARD", (char*)"PSEUDOCORR");
		if (k)
...
//		double buff;
	...
	// if the pseudo go in ordered, it always works
		...
		for (long long i = 0; i < n_pseudo_hard; i++)
		{
