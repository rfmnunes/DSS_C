...
public:
	...
	Zones_Pars(registry *reg, std::string sufix)
	{
	 // includes the variogram, bhist and hardata
		...
		k = get_key(reg, (char*)"ZONES", (char*)"NZONES");
		if (k)
			n_zones = get_int(k);
		else 
			exit(-99);
		...
		if (n_zones>1)
		{
			...
			if ((k = get_key(reg, (char*) "ZONES", (char*)"ZONESFILE")) != NULL)
				zonefl = get_string(k);
	...
	inline int get_zone_from_index(long long index)
	{
...
// now allocate and read zone-dependent parameters - harddata, bihist, variogram...
	...
	*/
	...
	template <class templt> void get_zonal_avg_var(GSLibGrid<templt>& cube_in)
	{
		...
		double av = 0;
		long long nxyz = cube_in.get_nxyz();
		int currzone;
