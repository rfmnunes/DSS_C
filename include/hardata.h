...
public:
	...
	HarddataPars(registry_type *reg, std::string keyword, int sufix) { 
		...
		if ((k = get_key(reg, (char*)group_id.c_str(), (char*)"DATAFILE")) != NULL)
			datafl = get_string(k);
		...
		k = get_key(reg, (char*)group_id.c_str(), (char*)"COLUMNS");
		if (k)
			nvari = get_int(k);
		else
			exit(-99);

		k = get_key(reg, (char*)group_id.c_str(), (char*)"XCOLUMN");
		if (k)
			ixl = get_int(k);
		else exit(-99);
		if (ixl < 0 || ixl > nvari) {
		...
		k = get_key(reg, (char*)group_id.c_str(), (char*)("YCOLUMN"));
		if (k)
			iyl = get_int(k);
		else exit(-99);
		if (iyl < 0 || iyl > nvari) {
		...
		k = get_key(reg, (char*)group_id.c_str(), (char*)("ZCOLUMN"));
		if (k)
			izl = get_int(k);
		else exit(-99);
		if (izl < 0 || izl > nvari) {
		...
		k = get_key(reg, (char*)group_id.c_str(), (char*)("VARCOLUMN"));
		if (k)
			ivrl = get_int(k);
		else exit(-99);
		...
		k = get_key(reg, (char*)group_id.c_str(), (char*)("WTCOLUMN"));
		if (k)
			iwt = get_int(k);
		else exit(-99);
		if (iwt < 0 || iwt > nvari) {
		...
		k = get_key(reg, (char*)group_id.c_str(), (char*)("MINVAL"));
		if (k)
			tmin = get_double(k);
		else exit(-99);

		k = get_key(reg, (char*)group_id.c_str(), (char*)("MAXVAL"));
		if (k)
			tmax = get_double(k);
		else exit(-99);
		...
		k = get_key(reg, (char*)group_id.c_str(), (char*)("USETRANS"));
		if (k)
			itrans = get_int(k);
		else exit(-99);

		if ((k = get_key(reg, (char*)group_id.c_str(), (char*)("TRANSFILE"))) != NULL)
		{
		...
		k = get_key(reg, (char*)keyword.c_str(), (char*)("ZMIN"));
		if (k)
			harddata_distr.zmin = get_double(k);
		else exit(-99);

		k = get_key(reg, (char*)keyword.c_str(), (char*)("ZMAX"));
		if (k)
		...
		k = get_key(reg, (char*)keyword.c_str(), (char*)("LTAIL"));
		if (k)
			ltail = get_int(k);
		else exit(-99);

		k = get_key(reg, (char*)keyword.c_str(), (char*)("LTPAR"));
		if (k)
			ltpar = get_double(k);
		else exit(-99);

		k = get_key(reg, (char*)keyword.c_str(), (char*)("UTAIL"));
		if (k)
			utail = get_int(k);
		else exit(-99);

		k = get_key(reg, (char*)keyword.c_str(), (char*)("UTPAR"));
		if (k)
			utpar = get_double(k);
		else exit(-99);
...
int read_harddata(misc& utils)
	...
	void Dtonode(GSLibGridPars& grid_def, misc& utils, boost::shared_array<float>& sim, GSLibGrid<int>& zones, int current_zone, int n_zones)
	{
		...
		for (unsigned long id = 1; id < point.size(); ++id)
		{
			...
			cl_coord<double> minus = xxyyzz - point[id].xyz;

			double dist = (double)fabs(minus.x) + (double)fabs(minus.y) + (double)fabs(minus.z);
			...
			if (sim[ind] != utils.nosvalue)
				if ((current_zone == zones.grid[ind]) || (n_zones <= 1))
				{
					...
					cl_coord<double> minus2 = xxyyzz - point[id2].xyz;
					double dist2 = (double)fabs(minus2.x) + (double)fabs(minus2.y) + (double)fabs(minus2.z);
