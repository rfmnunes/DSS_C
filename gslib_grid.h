...
//	std::vector<unsigned int>  cumulative_dims;	
//	//cum_dims is 
//		//[0] = nx
//		//[1] = nxy
//		//[2] = nxyz
//		//[3] = nxyzt ...
//	
//	long get_index_from_ijk_f(std::vector<unsigned int> ijk_coordinate)
//	{		
//
//		long previous = 1; 
//		// previous = 1 gives the fortran, 
//		//0 gives c if ijk starts on 1
//		previous = previous + ijk_coordinate[0] - 1); // -1 because starts on 1
//		
//		for (unsigned int i = 1; i < ijk_coordinate.size(); i++)
//			previous = previous + (ijk_coordinate[i] - 1)*cumulative_dims[(i - 1)];
//
//		return previous;
//
//	};
//	void get_ijk_from_index_f(long index,std::vector<unsigned int> ijk_coordinate){// unsigned int *ix, unsigned int *iy, unsigned int *iz){
//
//		long remainder = index;
//		
//		for (unsigned int i = n_dims-1;i > 0; i--)
//		{
//			ijk_coordinate[i]= ((remainder-1)/ cumulative_dims[i-1]) + (1);//cause start on 1
//			remainder = (remainder) % cumulative_dims[i-1];
//		}
//
//		ijk_coordinate[0] = remainder - 1 + 1; // check -1 and +1
//
//
//	}; // gets ijk coord form index 
//
//
//};
...
public:
	...
	unsigned long get_index_from_ijk_f(const cl_coord<int>& in_struct) {
		return (in_struct.x + (in_struct.y - 1)*nx + (in_struct.z - 1)*nxy);
	}; // get array index from the block coordinates 

	unsigned long get_index_from_xyz_f(const cl_coord<double>& in_struct) {

		cl_coord<int> temp_ijk = get_ijk_form_xyz_f(in_struct);
		unsigned long index = get_index_from_ijk_f(temp_ijk);
	...
	cl_coord<int> get_ijk_from_index_f(unsigned long index) {
	...
	cl_coord<double> get_xyz_from_index_f(unsigned long indx) {
	...
	// C style indexes - start on 0 
	//TO DO MAKE THIS COORD CLASS BASED
	//long get_index_from_ijk_c (unsigned int ix, unsigned int iy, unsigned int iz){
	//	return ((ix-1) + (iy-1)*nx + (iz-1)*nxy);
	//}; // get array index from the block coordinates 
	//void get_ijk_from_index_c(int index, unsigned int *ix, unsigned int *iy, unsigned int *iz){
	//	*iz = ((index)/nxy) + 1;
	//	*iy = ((index-(*iz-1)*nxy)/nx) + 1;
	//	*ix = index - (*iz-1)*nxy - (*iy-1)*nx + 1;
	//}; // gets ijk coord form index 
	//void get_xyz_from_ijk_c(unsigned int ix ,unsigned int iy,unsigned int iz, float *xcord,float *ycord,float *zcord ){
	//	*xcord = ox + (ix-1)*xsiz;
	//	*ycord = oy + (iy-1)*ysiz;
	//	*zcord = oz + (iz-1)*zsiz;
	//};
	//void get_ijk_form_xyz_c (float xcord,float ycord,float zcord, long *index_x, long *index_y, long *index_z) //gets index of coord in ijk
	//{
	//	// Compute the index of xcord
	//	*index_x = (long) ((xcord-ox)/xsiz + 1.5);
	//	// Check to see if in or out
	//	if (*index_x < 1)
	//		*index_x  = 1;
	//	else if (*index_x > nx)
	//		*index_x  = nx;
	//	// Compute the index of ycord
	//	*index_y = (long) ((ycord-oy)/ysiz + 1.5);
	//	// Check to see if in or out
	//	if (*index_y < 1)
	//		*index_y  = 1;
	//	else if (*index_y > ny)
	//		*index_y  = ny;
	//	// Compute the index of zcord
	//	*index_z = (long) ((zcord-oz)/zsiz + 1.5);
	//	// Check to see if in or out
	//	if (*index_z < 1)
	//		*index_z  = 1;
	//	else if (*index_z > nz)
	...
	//	return;
	//}
	//void get_xyz_from_index_c(unsigned long indx, float *xx ,float *yy,float *zz){
	//	unsigned int ix, iy, iz;
	//	get_ijk_from_index_c(indx, &ix, &iy, &iz);
	//	get_xyz_from_ijk_c(ix ,iy,iz, xx, yy, zz);
	//}; // get the block coordinates from the array index 
	//long get_index_from_xyz_c(float x, float y, float z){
	//	long ix,iy,iz;
	//	get_ijk_form_xyz_c ( x, y,z,&ix,&iy,&iz);
	//	return ((ix-1) + (iy-1)*nx + (iz-1)*nxy);
	//};
	...
	inline unsigned long get_nxy() { return nxy; };
	inline unsigned long get_nxyz() { return nxyz; };
...
//
//	// constructor
//	GSLibGridPars_gencoord() { }
//
//	GSLibGridPars_gencoord(registry_type *reg)
//	{//get grid parameters from registry.
//
//		reg_key *k;
//
...
//	};
//
//	GSLibGridPars_gencoord(GSLibGridPars_gencoord& grid_pars)
//	{
//		nx = grid_pars.nx; ny = grid_pars.ny; nz = grid_pars.nz;//number
//		ox = grid_pars.ox; oy = grid_pars.oy; oz = grid_pars.oz;//origin
//		xsiz = grid_pars.xsiz; ysiz = grid_pars.ysiz; zsiz = grid_pars.zsiz; //size
...
//	};
//
//	GSLibGridPars_gencoord(int nx, int ny, int nz, float ox, float oy, float oz, float xsiz, float ysiz, float zsiz)
//	{
//		this->nx = nx; this->ny = ny; this->nz = nz;//number
//		this->ox = ox; this->oy = oy; this->oz = oz;//origin
//		this->xsiz = xsiz; this->ysiz = ysiz; this->zsiz = zsiz; //size
...
//	// generic cooords
//	//===========================================================
//	unsigned long get_index_from_ijk_f (CoordinatesClass<int> *in_struct)
//	{
//		long index=0;
//		//do stuff
//		for(unsigned  int i = 1 ; i < in_struct->n_dimensions;++i)
//		{
//			index += (in_struct->coord[i]-1) * cumulative_blocks->coord[i-1];	
//		}
//		//do the first coordinate
//		index += in_struct->coord[0];
//		return index;
//	}; // get array index from the block coordinates 
//	   
//	unsigned long get_index_from_xyz_f(CoordinatesClass<float> *in_struct)
//	{
//		CoordinatesClass<int> temp_ijk = get_ijk_form_xyz_f ( in_struct);
//		unsigned long index = get_index_from_ijk_f(&temp_ijk);
//		return index;
//	}
//
//	CoordinatesClass<int> get_ijk_form_xyz_f (CoordinatesClass<float> *in_coord)
//	{
//	// computing indexes
//		CoordinatesClass<float> temp_coord = (*in_coord-*origin_blocks);
//		temp_coord  = (temp_coord / *size_blocks);
//	   	
//		CoordinatesClass<int> out_coord = (temp_coord);
//		for (unsigned int i = 0; i < in_coord->n_dimensions; ++i)
//		{
//	   		out_coord.coord[i] = out_coord.coord[i]+ 1.5;
//	   		if (out_coord.coord[i] < 1)
//	   		{
...
//	}
//
//	CoordinatesClass<int> get_ijk_from_index_for(unsigned long index)
//	{
//		CoordinatesClass<int> out_struct(number_dimensions);
//		for (unsigned int i = number_dimensions - 1; i >= 0; --i) 
//		{
//	   		out_struct.coord[i] = ((index - 1) / cumulative_blocks->coord[i - 1]) + 1;
//	   		get_ijk_from_index_for(index - (out_struct.coord[i] * cumulative_blocks->coord[i - 1]));
//		}
//	};
//
//	CoordinatesClass<float> get_xyz_from_ijk_f(CoordinatesClass<int> *in_struct) 
//	{
//		CoordinatesClass<int> str_ones(in_struct->n_dimensions);
//		for ( unsigned int i = 0; i < in_struct->n_dimensions; ++i)
//		{
//			str_ones.coord[i] = 1;
//		}
//		CoordinatesClass<float> out_struct(in_struct->n_dimensions);
//		out_struct  = (*origin_blocks) + (*in_struct - str_ones)* (*size_blocks);
//		return out_struct;
//	};
//
//	CoordinatesClass<float> get_xyz_from_index_for(unsigned long indx)
//	{
//		CoordinatesClass<int> ijk_struct(number_dimensions);
//		CoordinatesClass<float> out_struct(number_dimensions);
//		ijk_struct = get_ijk_from_index_for(indx);
//		out_struct = get_xyz_from_ijk_f(&ijk_struct);
//		return out_struct;
//	}; 
//
//	   //some utils
//
//	bool is_ijk_inside(CoordinatesClass<int> ijk)
//	{
//		for (int i = 0; i < number_dimensions; ++i)
//		{
//			if (ijk.coord[i]<1 || ijk.coord[i]>n_blocks->coord[i]) 
//			{
//				return false;
//			}
//		}
...
//	};
//
//	bool is_xyz_inside(CoordinatesClass<float> xyz)
//	{
//		for (int i = 0; i < number_dimensions; ++i)
//		{
//			if (xyz.coord[i] < origin_blocks->coord[i] - size_blocks->coord[i] / 2 ||
//				xyz.coord[i] > origin_blocks->coord[i] + n_blocks->coord[i] * size_blocks->coord[i])
//			{
//				return false;
//			}
//		}
...
//
//	//===========================================================
//	// generic cooords sets and gets
//	//===========================================================
//
//	int set_nblocks(int curr_dim, int value)
//	{
//		if (value >= 1)
//		{
//			this->n_blocks->coord[curr_dim] = value;
//			return 0;
//		}
//		else
//			logger << "Set number of blocks gave an error! Check the input parameters!";
...
//	}
//
//	inline unsigned int get_nblocks(int curr_dim)
//	{
//		return n_blocks->coord[curr_dim];
//	}
//
//	int set_origin_blocks(int curr_dim, float value)
//	{
//		this->origin_blocks->coord[curr_dim] = value;
//		return 0;
//	}
//
//	inline float get_origin_blocks(int curr_dim)
//	{
//		return origin_blocks->coord[curr_dim];
//	}
//
//	int set_size_blocks(int curr_dim, float value)
//	{
//		if (value >= 0)
//		{
//			size_blocks->coord[curr_dim] = value;
//			return 0;
//		}
//		else
//			logger << "Set size of blocks gave an error! Check the input parameters!";
...
//	inline float get_origin_blocks(int curr_dim)
//	{
//		return size_blocks->coord[curr_dim];
//	}
//
//	int set_cumulative_blocks(int curr_dim, float value)
//	{
//		if (value >= 0)
//		{
//			cumulative_blocks->coord[curr_dim] = value;
//			return 0;
//		}
//		else
//			logger << "Something went wrong setting nxy!";
...
//	}
//
//	inline float get_cumulative_blocks(int curr_dim)
//	{
//		return cumulative_blocks->coord[curr_dim];
//	}
//
//
//
...
public:
	...
	int		valid_points; // points with valid data - nxyz if no nulls are used  set with read grid, or copy from somewhere
...
template <class templt>	 GSLibGrid<templt>::GSLibGrid(char* & file)//constructor from a sgems file 
	...
	for( unsigned int i = 0; i < n_vars; ++i ) {
		for (unsigned long j= 1;j<=get_nxyz();++j){
...
template <class templt>	 GSLibGrid<templt>::GSLibGrid(GSLibGrid<templt>& a)//constructor allocates memory for the grid (+1 for fortran) 
{
	...
	//grid[0] = -999.25; // just to mark the fortran type grids
	for (unsigned long i= 1; i <= get_nxyz(); i++)
	{
		grid[i] = a.grid[i]; 
	}
};
...
template <class templt> void GSLibGrid<templt>::read_grid(){ // reads grid from file and puts it on *grid
		...
		valid_points = 0;
		for (unsigned long l = 1; l <= nxyz; l++){
...
template <class templt> void GSLibGrid<templt>::read_grid_binary_sgems(){ // reads grid from file and puts it on *grid
		...
		for( unsigned int j = 0; j < n_vars; ++j ) 
		{
			...
			for (unsigned int i= 1;i<=get_nxyz();++i)
			{ 
...
template <class templt> void GSLibGrid<templt>::write_grid_ascii()
    {
		...
		long here_nxyz = get_nxyz();
        
		for (unsigned int i=1; i<here_nxyz+1;i++)
		{ 
...
{ // copy from a GSLibGridPars to a specific grid
	for (unsigned int i= 0; i <= get_nxyz(); i++) grid[i] = 0;
}
