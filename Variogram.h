...
public:
	...
	int get_structure(registry_type *reg, std::string group_id, int i) 
	{
		...
		if ((k = get_key(reg, (char*)variogram.c_str(), ("TYPE"))) == NULL) 
			return 1;
		model = get_int(k);
		...
		if ((k = get_key(reg, (char*)variogram.c_str(), ("COV"))) == NULL) 
			return 1;	
		...
		if ((k = get_key(reg, (char*)variogram.c_str(), ("ANG1"))) == NULL) 
			return 1;
		ang1 = get_float(k);
		logger << "Angle one: " << ang1 << "\n";

		if ((k = get_key(reg, (char*)variogram.c_str(), ("ANG2"))) == NULL) 
			return 1;
		ang2 = get_float(k);
		logger << "Angle two: " << ang2 << "\n";

		if ((k = get_key(reg, (char*)variogram.c_str(), ("ANG3"))) == NULL) 
			return 1;
		ang3 = get_float(k);
		logger << "Angle three: " << ang3 << "\n";

		if ((k = get_key(reg, (char*)variogram.c_str(), ("AA"))) == NULL) 
			return 1;
		max_amplitude = get_float(k);
		logger << "Amplitude one: " << max_amplitude << "\n";

		if ((k = get_key(reg, (char*)variogram.c_str(), ("AA1"))) == NULL) 
			return 1;
		aa1 = get_float(k);
		logger << "Amplitude two: " << aa1 << "\n";


		if ((k = get_key(reg, (char*)variogram.c_str(), ("AA2"))) == NULL) 
			return 1;
		aa2 = get_float(k);
...
public:
	...
	VariogramPars(registry_type *reg, std::string keyword, int sufix)
	{
		...
		k = get_key(reg, (char*)group_id.c_str(), ("NSTRUCT"));
		if (k)
			nst = get_int(k);
		else 
			exit(-99);
		...
		k = get_key(reg, (char*)group_id.c_str(), ("NUGGET"));
		if (k)
			c0 = get_float(k);
		else
			exit(-99);
		...
		k = get_key(reg, ("SEARCH"), ("RADIUS1"));
		if (k)
			radius = get_float(k);
		else 
			exit(-99);

		k = get_key(reg, ("SEARCH"), ("RADIUS2"));
		if (k)
			radius1 = get_float(k);
		else 
			exit(-99);

		k = get_key(reg, ("SEARCH"), ("RADIUS3"));
		if (k)
			radius2 = get_float(k);
		else 
			exit(-99);
		...
		k = get_key(reg, ("SEARCH"), ("SANG1"));
		if (k)
			search_radius->ang1 = get_float(k);
		else 
			exit(-99);

		k = get_key(reg, ("SEARCH"), ("SANG2"));
		if (k)
			search_radius->ang2 = get_float(k);
		else 
			exit(-99);

		k = get_key(reg, ("SEARCH"), ("SANG3"));
		if (k)
		...
		k = get_key(reg, ("COVTAB"), ("MAXCTX"));
		if (k)
			MAXCT.x = get_int(k);
		else 
			exit(-99);
		k = get_key(reg, ("COVTAB"), ("MAXCTY"));
		if (k)
			MAXCT.y = get_int(k);
		else 
			exit(-99);
		k = get_key(reg, ("COVTAB"), ("MAXCTZ"));
		if (k)
			MAXCT.z = get_int(k);
		else 
			exit(-99);
